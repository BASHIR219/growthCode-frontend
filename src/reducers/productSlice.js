...
export const createProduct = createAsyncThunk(
  ...
  async (ProductData, thunkAPI) => {
    try {
      ...
      const response = await axios.post('http://localhost:4000/api/v1/admin/products/new', ProductData,
        {
...
export const fetchHomeProducts = createAsyncThunk(
  ...
  async (page=1, { rejectWithValue }) => {
    try {
      const response = await axios.get(`http://localhost:4000/api/v1/products?page=${page}`);
...
export const fetchProductsByName = createAsyncThunk(
  ...
  async ({ keyword = "", page = 1, priceVal=[0, 123456], category="" }, { rejectWithValue }) => {
    try {
      ...
      // let link = `http://localhost:4000/api/v1/products?name=${keyword}&page=${page}}`

      let link = `http://localhost:4000/api/v1/products?page=${page}&name=${keyword}&gte=${priceVal[0]}&lte=${priceVal[1]}&category=${category}`
...
export const fetchProductsByKeyword = createAsyncThunk(
  ...
  async (keyword="", { rejectWithValue }) => {
    try {
      ...
      let link = `http://localhost:4000/api/v1/products?name=${keyword}`
...
export const deleteProduct = createAsyncThunk(
  ...
  async (id, thunkAPI) => {
    try {
      const response = await axios.delete(`http://localhost:4000/api/v1/admin/products/${id}`,{
        withCredentials:true
      });
      return response.data;
    } catch (error) {
